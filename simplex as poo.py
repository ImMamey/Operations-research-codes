...
def getSizeConds(restrict, cond)->int:
...
def menorIgual(m_aum: any, z:any, cr:any)->None:
    ...
    if max >= 0:  # parada
        ...
        for i in range(cantColumns):
            print(
                "nombre y tipo ", m_aum[i].name, m_aum[i].base, "m_aum=", m_aum[i].arr
            )
...
if "__main__" == __name__:
    z = np.array([60, 30, 20])
    cr = np.array([48, 20, 8])
    restrict = np.array([[8, 4, 2], [6, 2, 1], [2, 1.5, 0.5]])
    cond = ["<=", "<=", "<="]
